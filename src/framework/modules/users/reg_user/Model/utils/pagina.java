...
public class pagina {
    ...
    public static void inicializa() {
        ...
        rowCount = ((miniSimpleTableModel_reg)reg_table.TABLA.getModel()).getRowCount();
        ...
        reg_table.jPanel4.setLayout(new BorderLayout());
        reg_table.jPanel4.add(pagina.box);
                    
    }
    public static void initLinkBox() {
        
                    reg_table.sorter.setRowFilter(new RowFilter<TableModel, Integer>() {
        ...
        if(  (rowCount<=itemsPerPage) && (rowCount>0)  ){ //caben todos los datos en la misma página
                ...
                reg_table.primero.setEnabled(false);
                reg_table.ANTERIOR.setEnabled(false);
                reg_table.SIGUIENTE.setEnabled(false);
                reg_table.ultimo.setEnabled(false);
                reg_table.CAJA.setText("");
            ...
            box.repaint();
                    ...
                    reg_table.primero.setEnabled(currentPageIndex>1);
                    reg_table.ANTERIOR.setEnabled(currentPageIndex>1);
                    reg_table.SIGUIENTE.setEnabled(currentPageIndex<maxPageIndex);
                    reg_table.ultimo.setEnabled(currentPageIndex<maxPageIndex);
                    reg_table.CAJA.setText(Integer.toString(currentPageIndex) + String.format(" / %d", maxPageIndex));
